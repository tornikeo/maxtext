...
def reshard_fn(config: pyconfig.HyperParameters):
  ...
  def reshard(tree, mesh=mesh):
    ...
    return config.eu.reshard(tree, shardings, donate=True)
  ...
  state = state.replace(step=step, params=params, opt_state=opt_state)
      ...
      with mesh, nn_partitioning.axis_rules(config.logical_axis_rules), elasticutils.watchdog(120):
        ...
        with jax.profiler.StepTraceAnnotation("train", step_num=step):
          ...
          if step % config.eu.save_period == 0:
            jax.block_until_ready(state)
            config.eu.save({'state': jax.tree.map(lambda x: x.copy(), state)})

        new_time = datetime.datetime.now()
        record_scalar_metrics(
            metrics, new_time - last_step_completion, per_device_tflops, learning_rate_schedule(state.step), per_device_tokens
        )
