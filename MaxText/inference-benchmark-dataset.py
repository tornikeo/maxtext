...
def get_quantiles(name, stats_array, n=10):
...
def benchmark_prefill_insert(config, engine, decode_state, params, tokens,
                             true_lengths, start_row, end_row):
...
def benchmark_autoregressive(config, engine, decode_state, params, start_row, end_row):
...
def benchmark(config, engine, decode_state, params, tokens, true_lengths,
    ...
    batch_stats[_AUTOREGRESSIVE], output_tokens = benchmark_autoregressive(
      config, engine, decode_state, params, start_row, end_row
      )
...
def aggregate_stats(stats_list):
  ...
  for k_out in output_stats.keys():
    if len(output_stats[k_out]) > 1:
      result[k_out] = get_quantiles(k_out, output_stats[k_out])
    else:
...
def main(config):
  ...
  for start_row in range(0, dataset_size, batch_size):
    ...
    batch_stats, output_tokens = benchmark(config, engine, decode_state, params, prefill_tokens, prefill_true_lengths, start_row, end_row)
    ...
    for k in batch_stats.keys():
      ...
      logging.debug(f'\nAccuracy: {rouge_val}')
    batch_processed += 1
    if config.profiler != "" and _NUM_PROFILES > 0 and _NUM_PROFILES == batch_processed:
      logging.info("NUM_PROFILES {_NUM_PROFILES} reached - not iterating over remaining dataset")
      break
  ...
  print(get_quantiles('predicted_str_len', len_pred_text))
  print(get_quantiles('target_str_len', len_target_text))
  logging.info('\nEnd')
